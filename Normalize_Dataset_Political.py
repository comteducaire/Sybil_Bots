...
def Normalize_Topics_Lists(Statuses_List, word_map, topic_num):
    ...
    while (i < len(Statuses_List)):
        ...
        for word in All_Words_in_Statuses_Single_User:
            similarity = nlp(word).similarity(nlp(word_map[topic][0]))
            match_score += similarity
            #if word in word_map[topic]:

        if len(All_Words_in_Statuses_Single_User) != 0:
            Normalized_List.append((float(match_score) / len(All_Words_in_Statuses_Single_User)))
        else:
...
with open('Normalized_Political_3Topics.csv', 'w+', newline='',  encoding='utf-8') as csvfile_clean:
    ...
    writer.writerow(['ID', 'Screen_Name','Length_of_UserName', 'Followers_to_Friends_Ratio', 'Posting_Rate'
                     ...
                     'Followers', 'Statuses', 'Topic 1', 'Topic 2', 'Topic 3',])
    ...
    Topic1_List = Normalize_Topics_Lists(Statuses_List, word_map, 1)
    Topic2_List = Normalize_Topics_Lists(Statuses_List, word_map, 2)
    Topic3_List = Normalize_Topics_Lists(Statuses_List, word_map, 3)
    #Topic4_List = Normalize_Topics_Lists(Statuses_List, word_map, 4)
    #Topic5_List = Normalize_Topics_Lists(Statuses_List, word_map, 5)
    #Topic6_List = Normalize_Topics_Lists(Statuses_List, word_map, 6)
    #Topic7_List = Normalize_Topics_Lists(Statuses_List, word_map, 7)
    ...
    while i < len(IDs_List):
        writer.writerow([IDs_List[i], Screen_Name_List[i],Length_of_UserName_List[i],Followers_to_Friends_Ratio_List[i],
                         ...
                         Friends_List[i],Followers_List[i],Statuses_List[i], Topic1_List[i], Topic2_List[i],
                        Topic3_List[i]])
        i += 1
